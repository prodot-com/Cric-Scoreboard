...
<div className="font-mono min-h-screen bg-black text-white flex flex-col items-center relative">
  {/* Navbar */}
  <div
    className="fixed top-5 sm:top-5 border-2 border-amber-600 bg-neutral-900/90 
    w-[92%] sm:w-[90%] max-w-[1330px] 
    flex flex-col sm:flex-row justify-between items-center px-4 sm:px-10 py-3 sm:py-4 
    rounded-2xl z-50 backdrop-blur-sm shadow-2xl"
  >
    <h1
      onClick={() => window.location.reload()}
      className="text-lg sm:text-3xl font-bold text-white cursor-pointer text-center sm:text-left"
    >
      CricScoreBoard
    </h1>

    <div className="flex flex-col sm:flex-row items-center sm:space-x-6 mt-2 sm:mt-0 w-full sm:w-auto">
      {/* Icons */}
      <div className="flex items-center space-x-6 justify-center sm:justify-start w-full sm:w-auto">
        <a
        ...
        <a
          ...
          className="hover:text-amber-600 transition"
        >
          <Mail className="w-5 h-5 sm:w-6 sm:h-6 pointer-events-auto" />
        </a>
      </div>

      <div className="hidden sm:block font-bold px-2 py-1 text-[16px] sm:text-[22px] text-neutral-500">
        <LiveTime />
      </div>
    </div>
  </div>
  ...
  {/* Main Content */}
  <div className="relative z-10 w-full flex justify-center mt-28 sm:mt-24 px-3">
    <div
      className="w-full
      backdrop-blur-lg shadow-2xl rounded-2xl p-5 sm:p-8 flex flex-col sm:flex-row items-center gap-6 sm:gap-8"
    >
      {/* Left: Image */}
      <div className="flex justify-center items-center w-full sm:w-1/3">
        <img
          ...
          w-[300px] sm:w-[280px] lg:w-[300px] h-auto object-contain drop-shadow-2xl rounded-xl"
        />
      </div>

      {/* Right: Toss UI */}
      <div className="border-2 border-amber-600 bg-neutral-800/67 sm:mr-16 rounded-2xl h-full flex flex-col items-center justify-center w-full sm:w-2/3 p-4 sm:p-6">
        <h1 className="text-2xl sm:text-3xl font-bold text-amber-500 mb-4 sm:mb-6">
          Toss Time 🎲
        </h1>

        {/* Coin */}
        <div className="w-24 h-24 sm:w-32 sm:h-32 perspective mb-6">
          {!tossWinner ? (
            <div
            ...
            >
              <div className="absolute w-full h-full bg-amber-400 rounded-full flex items-center justify-center font-bold text-black text-lg sm:text-xl backface-hidden">
                {matchData.team1}
              </div>
              <div
                className="absolute w-full h-full bg-blue-600 rounded-full flex items-center justify-center font-bold text-white text-lg sm:text-xl backface-hidden"
            ...
            <div className="absolute w-full h-full bg-emerald-500 rounded-full flex items-center justify-center font-bold text-white text-lg sm:text-xl shadow-lg">
              {tossWinner}
            </div>
          )}
        </div>
        ...
        {tossWinner && (
          <div className="mt-6 flex flex-col items-center gap-3 sm:gap-4">
            <h2 className="text-xl sm:text-2xl font-bold text-amber-400 animate-bounce">
            ...
            {decision && (
              <h3 className="mt-4 text-base sm:text-lg text-gray-200 text-center">
                {tossWinner} chose to{" "}
  ...
  {/* 🔹 Footer */}
<footer className="mt-auto py-3 text-center text-gray-400 text-xs sm:text-sm flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-4">
  <span>©2025 CricScoreBoard | Built with 🧡</span>
</footer>
