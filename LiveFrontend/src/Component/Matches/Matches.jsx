...
import { Github, Linkedin, Mail, PlusCircle, Clock } from 'lucide-react';

// Live Time Component for the Navbar
const LiveTime = () => {
...
const LoadingSpinner = () => (
  <div className="flex justify-center items-center space-x-2">
    ...
    <div className="w-4 h-4 rounded-full animate-pulse bg-amber-500 delay-200"></div>
    <div className="w-4 h-4 rounded-full animate-pulse bg-amber-500 delay-400"></div>
    <span className="ml-2 text-neutral-400">Loading Matches...</span>
  </div>
);
...
const Matches = () => {
  ...
  const API_URL = 'http://localhost:9000/user/get';

  useEffect(() => {
    const fetchMatch = async () => {
      try {
        const response = await axios.get(API_URL);
        // Sort matches to show live ones first, then completed ones
    ...
    <div className='font-mono bg-black text-white min-h-screen w-full flex flex-col'>
      ...
      <header className="fixed top-0 sm:top-5 left-0 right-0 flex justify-center z-50 pointer-events-none">
        <div className="border-b sm:border-2 border-amber-600 bg-neutral-900/90 w-full sm:w-[95%] sm:max-w-[1330px] flex justify-between items-center px-4 sm:px-10 py-4 sm:rounded-2xl backdrop-blur-sm shadow-2xl pointer-events-auto">
          <h1 onClick={() => routeChange('/')} className="text-xl sm:text-3xl font-bold text-white cursor-pointer">
            CricScoreBoard
          </h1>
          <div className="flex items-center space-x-2 sm:space-x-5">
           
             <button onClick={() => routeChange('/')} className="hidden sm:flex items-center space-x-2 text-neutral-300 hover:text-amber-500 transition">
      ...
      <main className='w-full relative pt-24 sm:pt-32 pb-10 px-4 flex-grow'>
        <div
          ...
          style={{ backgroundImage: `
            radial-gradient(circle at 50% 0%, rgba(217, 119, 6, 0.4) 0%, transparent 50%),
            radial-gradient(circle at 10% 20%, rgba(217, 119, 6, 0.2) 0%, transparent 40%),
            radial-gradient(circle at 90% 80%, rgba(217, 119, 6, 0.2) 0%, transparent 40%)
          `}}
        />
        ...
        <div className='relative z-10 flex flex-col items-center w-full'>
          ...
          {loading ? (
            ...
            <div className="bg-neutral-900/80 border border-neutral-700 rounded-xl p-8 text-center">
              ...
              <button onClick={() => routeChange('/')} className="bg-amber-600 hover:bg-amber-500 text-white font-semibold py-2 px-5 rounded-lg transition">Create Match</button>
            </div>
          ) : (
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 w-full max-w-7xl'>
              {matches.map((match) => (
                <div 
                  key={match._id} 
                  onClick={() => routeChange(`/Live/${match._id}`)} 
                  className={`group relative overflow-hidden bg-neutral-900/80 backdrop-blur-md border p-6 rounded-2xl shadow-lg 
                             transition-all duration-300 cursor-pointer
                             ${match.completed 
                               ? 'border-neutral-700 hover:bg-neutral-800/60' 
                               : 'border-amber-500/30 hover:border-amber-500 hover:scale-105 hover:shadow-2xl hover:shadow-amber-500/20'
                             }`}
                  style={{
                    backgroundImage: `radial-gradient(circle at 20% 20%, rgba(255,255,255,0.03), transparent), 
                                      radial-gradient(circle at 80% 80%, rgba(255,255,255,0.03), transparent)`
                  }}
                >
                  ...
                  <div className='flex flex-col items-center text-center'>
                    {match.completed && match.firstSummary && match.secondSummary ? (
                      <>
                        <div className="w-full flex justify-between items-baseline"><h2 className='text-lg sm:text-xl font-bold text-white'>{match.firstSummary.battingTeam}</h2><p className="text-base sm:text-lg font-semibold text-neutral-300">{match.firstSummary.runs}</p></div>
                        <h1 className='text-sm sm:text-md font-medium text-neutral-500 my-1'>vs</h1>
                        <div className="w-full flex justify-between items-baseline"><h2 className='text-lg sm:text-xl font-bold text-white'>{match.secondSummary.battingTeam}</h2><p className="text-base sm:text-lg font-semibold text-neutral-300">{match.secondSummary.runs}</p></div>
                      </>
                    ) : (
                      <>
                        <h2 className='text-xl sm:text-2xl font-bold text-white'>{match.team1}</h2>
                        <h1 className='text-lg sm:text-xl font-medium text-amber-500 my-2'>vs</h1>
                  ...
                  <div className="text-center mt-4 h-6">
                    {match.completed ? (
                       <p className="text-sm text-green-400 font-bold">{getWinner(match)}</p>
                    ) : (
                       <div className="font-semibold text-amber-600 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                         Watch Now &rarr;
                       </div>
                    )}
                  </div>
      ...
      <footer className="w-full mt-auto py-2">
        <div className="max-w-7xl mx-auto px-4 sm:px-10 flex flex-col sm:flex-row justify-center items-center gap-6">
          <div className="text-center sm:text-left">
            <p className="text-sm text-neutral-400 mt-1">©{new Date().getFullYear()} | Built with 🧡 by Probal Ghosh</p>
          </div>
